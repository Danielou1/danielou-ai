...
impl TimeoutConfig {
    ...
    /// - `DANIELOU_API_CONNECT_TIMEOUT` or `CLAW_API_CONNECT_TIMEOUT` — connect timeout in seconds
    /// - `DANIELOU_API_REQUEST_TIMEOUT` or `CLAW_API_REQUEST_TIMEOUT` — overall request timeout in seconds
    #[must_use]
    pub fn from_env() -> Self {
        let connect_timeout = std::env::var("DANIELOU_API_CONNECT_TIMEOUT")
            .or_else(|_| std::env::var("CLAW_API_CONNECT_TIMEOUT"))
            .ok()
            .and_then(|v| v.parse::<u64>().ok())
            .map(Duration::from_secs)
            .unwrap_or(Duration::from_secs(30));
        let request_timeout = std::env::var("DANIELOU_API_REQUEST_TIMEOUT")
            .or_else(|_| std::env::var("CLAW_API_REQUEST_TIMEOUT"))
            .ok()
            .and_then(|v| v.parse::<u64>().ok())
            .map(Duration::from_secs)
